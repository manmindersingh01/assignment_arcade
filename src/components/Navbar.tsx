...
import { PaintBucket, MenuIcon } from "lucide-react"
...
const Navbar = () => {
  ...
  return (
    <>
      <div
        className={cn(
          "fixed top-5 inset-x-0 mx-2 md:mx-5 z-50 rounded-lg bg-background flex items-center justify-between p-4 transition-all duration-300",
          scrolled ? "border border-border" : "border-transparent",
        )}
      >
        {/* Logo */}
        <div className="flex items-center space-x-2 md:pl-10">
          <PaintBucket size={24} />
          <h1 className="font-extrabold text-xl md:text-2xl">ARCADE</h1>
        </div>
        ...
        {/* Desktop Menu */}
        <div className="hidden md:flex items-center space-x-4 hidden md:block">
          <Menu>
          ...
          <div className="flex items-center gap-4">
            <ModeToggle />
            <Button className="p-4 text-lg font-normal text-gray-700" variant="ghost">
              Pricing
            </Button>
            <Button className="p-4 text-lg font-normal text-gray-700" variant="ghost">
              Log in
      ...
      {/* Mobile Sidebar */}
      <Mobile isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} />
    </>
  )
}
